...
for type1 in faultType:
    ...
    print(type1, "处理前代码片段总量为：", data_len1, data_len2, data_len3, data_len2)
    ...
    for i in range(data_len1):
        ...
        data_neg_patch = (dataset_pkl[type1]["negative"][i]).replace(
            "rank2fixstart", " ").replace("rank2fixend", " ")
        ...
        if(len(token_seq) <= 200):
            # 原始有错误标记的positive
            code_list_pos.append(dataset_pkl[type1]["positive"][i])
            code_list_neg.append(data_neg)  # 原始有错误 fl的 negative
            code_list_pos_pat.append(
                dataset_pkl[type1]["patch"][i])  # 修复后的patch
...
for type1 in faultType:
    for type1_1 in codeType:
        ...
        file = open(file_path + '/src_token_' + type1_1 +
        ...
        count = len(open(file_path + '/src_token_' +
                    type1_1 + '.txt', 'r').readlines())
        print("处理后" + type1 + '的' + type1_1 + "文件的长度度为：", count)
